...
function New() {
    window.location.replace("/EmployeeMaster/Index/0");
}

function List() {
    $("#EmployeeMasterGridList").css("display", "block");
    $("#myModal").modal("show");
    $("#EmployeeMasterGridList").appendTo("#modalList");
    $("#myModal").show();
}
...
function Build() {
    var obj = new Object();
    obj.EmployeeMaster_ID = $('#hdEmployeeMaster_ID').val();
    obj.EmployeeCodeTemplate = $('#EmployeeCodeTemplate').val();
    obj.EmployeeCode = $('#EmployeeCode').val();
    obj.EmployeeName = $('#EmployeeName').val();
    obj.EmployeeType = $('#EmployeeType').val();
    obj.DOB = $('#DOB').val();
    obj.Gender = $('#Gender').val();
    obj.JointDate = $('#JointDate').val();
...
function Save() {
    var build = Build();
    if (build.EmployeeCode == "" || build.EmployeeCode == null || build.EmployeeCode == "0") {
        toastr.error("Employee Code Cannot be empty");
        $('#EmployeeCode').focus();
        return false;
    }
    else if (build.EmployeeName == "" || build.EmployeeName == null) {
        toastr.error("Employee Name Cannot be empty");
        $('#EmployeeName').focus();
        return false;
    }
    else if (build.EmployeeType == "" || build.EmployeeType == null || build.EmployeeType == "0") {
        toastr.error("Please Choose EmployeeType");
        $('#DisplayName').focus();
        return false;
    }
    ...
    else if (build.JointDate == "" || build.JointDate == null) {
        toastr.error("JointDate Cannot be empty");
        $('#JointDate').focus();
        return false;
    }
    else {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.ReturnValue == 1) {
                    ...
                    window.location.replace("/EmployeeMaster/Index/" + data.EmployeeMaster_ID);
                }
...
function Delete() {
    var build = Build();
    if (build.EmployeeMaster_ID == "0") {
        toastr.error('EmployeeMaster Cannot be empty');
        return false;
    }
    else {
...
$(document).ready(function () {
    ...
    if ($('#hdEmployeeMaster_ID').val() > 0)
    {
        flatpickr("#JointDate", {
            enableTime: true,
            defaultDate: Date.parse($('#hdJointDate').val())
        });
    ...
    $.ajax({
        type: "POST",
        url: '/EmployeeMaster/GetEmployeeCodeType',
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ "searchTerm": null }),
        dataType: "json",
        success: function (data) {
            $("#EmployeeCodeTemplate").select2({
                data: data
            });
            //$("#EmployeeCodeTemplate").val($('#hdEmployeeCodeTemplate').val()).trigger("change");
        },
        error: function (data) {
        }
    });
    ...
    $.ajax({
        type: "POST",
        url: '/EmployeeMaster/GetEmployeeType',
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ "searchTerm": null }),
        dataType: "json",
        success: function (data) {
            ...
            $("#EmployeeType").val($('#hdEmployeeType').val()).trigger("change");
            $('#LoadingImg').hide();
        },
        error: function (data) {
        }
    ...
    var EmployeeMasterGrid;

    EmployeeMasterGrid = $("#EmployeeMasterGrid").grid({
        dataKey: "ID",
        uiLibrary: "bootstrap",
        columns: [
            { field: "EmployeeMaster_ID", title: "ID", hidden: true, width: '100%' },
            ...
            { field: "EmployeeType", sortable: true, width: '100%' },
            { field: "Gender",  sortable: true, width: '100%' },
            { field: "JointDate", hidden: true, sortable: true, width: '100%' },
    ...
    $("#btnSearchEmployeeMaster").on("click", SearchEmployeeMaster);
    ...
    if ($('#hdEmployeeMaster_ID').val() > 0) {
        ...
        //Address
        AddressGrid.reload({ Ref_ID: $('#hdEmployeeMaster_ID').val(), CodeTempate: $('#hdEmployeeCodeTemplate').val() });
        ...
        //IDProof
        IDProofGrid.reload({ Ref_ID: $('#hdEmployeeMaster_ID').val(), CodeTempate: $('#hdEmployeeCodeTemplate').val() });
        $("#btnAddIDProof").on("click", AddIDProof);
        $("#btnIDProofSave").on("click", SaveIDProof);

        //StorageMaster
        StorageMasterGrid.reload({ Ref_ID: $('#hdEmployeeMaster_ID').val(), CodeTempate: $('#hdEmployeeCodeTemplate').val() });
...
function View(e) {
    var id = $('#EmployeeMasterGrid').grid().getById(e.data.id).EmployeeMaster_ID;
    if (id != null && id != 0) {
        window.location.replace("/EmployeeMaster/Index/" + id + "");
    }
}

function SearchEmployeeMaster() {
    $('#EmployeeMasterGrid').grid().reload({ searchString: $("#searchEmployeeMaster").val() });
}
...
function SaveAddress() {
    var addressMaster = {
        ...
        Email: $('#Email').val(),
        Ref_ID: $('#hdEmployeeMaster_ID').val(),
        CodeTemplate: $('#hdEmployeeCodeTemplate').val(),
    ...
    if (addressMaster.AddressType == null) {
        ...
        .done(function (data) {
            $('#LoadingImg').hide();
            $('#AddressGrid').grid().reload({ Ref_ID: $('#hdEmployeeMaster_ID').val(), CodeTempate: $('#hdEmployeeCodeTemplate').val() });
    ...
    if (confirm("Are you sure you want to delete?")) {
        ...
        .done(function (data) {
            ...
            if (data == true) {
                ...
                $('#AddressGrid').grid().reload({ Ref_ID: $('#hdEmployeeMaster_ID').val(), CodeTempate: $('#hdCodeTemplate').val() });
            }
        })
        .fail(function (data) {
            $('#LoadingImg').hide();
...
function SaveIDProof() {
    ...
    if (idProofMaster.IDProofType == null) {
        ...
        .done(function (data) {
            $('#LoadingImg').hide();
            $('#IDProofGrid').grid().reload({ Ref_ID: $('#hdEmployeeMaster_ID').val(), CodeTempate: $('#hdEmployeeCodeTemplate').val() });
...
function SaveStorageMaster() {
    ...
    if (storageMaster.StorageType == null) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#LoadingImg').hide();
                $('#StorageMasterGrid').grid().reload({ Ref_ID: $('#hdEmployeeMaster_ID').val(), CodeTempate: $('#hdEmployeeCodeTemplate').val() });
